...
import { cards, olderPassage } from "./data";
import { getWeakSpotCards } from "./hifzModel";
import { Days, MemorisationRange, SurahRange } from "./types";
    ...
    for (const item of plan) {
      await Notifications.scheduleNotificationAsync({
        content: {
          ...
          data: { screen: item.screen, ayah: item.ayah }
        },
        trigger: {
...
function buildNotificationPlan(settings: ReminderSettings) {
  ...
  const now = new Date();
  const maxItems = 48;
  const revisionRange = settings.revisionRanges.find((range) => range.id === settings.revisionTargetId) ?? settings.revisionRanges[0];
  const weakQueue = settings.weakOn ? getWeakSpotCards() : [];
  ...
  for (let dayOffset = 0; dayOffset < 14 && plan.length < maxItems; dayOffset += 1) {
    ...
    slots
      ...
      .forEach((slot, index) => {
        ...
        if (slot.type === "sabaq") {
          const card = cards[(index + dayOffset) % cards.length];
          plan.push({
            date,
          ...
          const dueWeak = weakQueue[(index + dayOffset) % Math.max(weakQueue.length, 1)];
          const ayah = olderPassage[(index + dayOffset) % olderPassage.length];
          plan.push({
            date,
            title: "Hifz Cards · Revision",
            body: dueWeak
              ? `${revisionRange?.label ?? "Revision range"}: begin with weak āyah ${dueWeak.card.ayahNumber}, then continue your revision.`
              : `${revisionRange?.label ?? "Revision range"}: start from āyah ${ayah.num}. How far can you continue?`,
            ayah: dueWeak ? dueWeak.card.ayahNumber : ayah.num,
            screen: dueWeak ? "session:weak" : "session:revision"
          });
        }
      });
  }
